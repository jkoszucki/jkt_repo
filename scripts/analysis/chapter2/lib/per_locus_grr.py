...
genomes (match, locus_only, pc_only) for each best-predictor locus.
...
Output per locus: per_locus_dir/{KL}/locus/grr_vs_reference.tsv
...
def export_grr_vs_reference(
    ...
    """
    For each locus in best_predictors.csv, compute wGRR between the reference
    GenBank and every genome in match/, locus_only/, pc_only/.

    Output: per_locus_dir/{KL}/locus/grr_vs_reference.tsv
        genomeID  category  wgrr  grr_sum  genes_ref  genes_genome  n_bbh
    """
    ...
    for _, row in best.iterrows():
        locus     = row["locus"]
        locus_dir = per_locus_dir / locus / "locus"
        ref_gb    = locus_dir / f"{locus}_reference.gb"
        out_tsv   = locus_dir / "grr_vs_reference.tsv"

        if not ref_gb.exists():
            print(f"  [{locus}] reference GB not found, skipping")
            continue
        ...
        if not category_genomes:
            print(f"  [{locus}] no category genomes found, skipping")
            continue

        n_total = len(category_genomes)
        print(f"  [{locus}] {n_total} genomes …", flush=True)

        # Extract proteins: REFERENCE + all category genomes
        ref_prots = _extract_proteins(ref_gb, REF_ID)
        if not ref_prots:
            print(f"  [{locus}] reference has no CDS, skipping")
            continue
