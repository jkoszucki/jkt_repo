...
from ktypes_api_base import BaseKTypeAPI
...
class KTypeStructureDrawer(BaseKTypeAPI):
    """Generate structural diagrams for K-types."""

    def __init__(self, *args, plots_subdir: str = "plots/ktypes_structures", **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._tables_api = KTypeTablesAPI(self.input_xlsx, self.output_dir)
    ...
    def _ensure_processed(self, df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        if df is not None:
            return df
        return self._tables_api.build_processed_table()

    def _ensure_modifications_table(self) -> pd.DataFrame:
        if self._modifications_df is None:
            try:
                df = self._tables_api.get_modifications()
            except KeyError:
                df = pd.DataFrame()
            df = df.copy()
            if not df.empty:
    ...
    def draw_all(self, df: Optional[pd.DataFrame] = None, *, overwrite: bool = True, test: bool = False) -> list[Path]:
        ...
        for _, row in processed.iterrows():
            kname = str(row.get("K_type", "")).strip()
    ...
    def draw_single(self, row: pd.Series, stem_name: Optional[str] = None) -> tuple[bool, list[Path]]:
        ...
        ax.set_title(f"{row.get('K_type', 'K?')}", fontsize=12, pad=12)
